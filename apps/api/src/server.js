...
function auth(req, res, next) {
    ...
    try {
        req.user = jwt.verify(h.replace("Bearer ", "").trim(), env.JWT_SECRET);
        return next();
    }
    catch {
...
const singleJobSchema = z
    ...
    if (modes > 1) {
        ctx.addIssue({
            ...
            message: "Use only one scheduling mode per request: delaySeconds, runAt, or cronExpression"
        });
    }
...
const updateQueueSchema = z
    ...
    if (hasRateLimitCount !== hasRateLimitWindowSec) {
        ctx.addIssue({
            code: z.ZodIssueCode.custom,
            message: "rateLimitCount and rateLimitWindowSec must be provided together when updating rate limits"
        });
    }
});
const updateScheduledJobSchema = z
    ...
    .refine((value) => Object.keys(value).length > 0, "At least one field must be provided for scheduled job update");
...
app.post("/api/v1/auth/login", async (req, res) => {
    try {
        ...
        const token = jwt.sign({ userId: user.id, email: user.email }, env.JWT_SECRET, { expiresIn: env.JWT_EXPIRES_IN });
        return res.json({
            token,
 ...
 * NEW: Create project
 */
app.post("/api/v1/projects", auth, async (req, res) => {
    try {
 ...
 */
app.post("/api/v1/projects/:projectId/queues", auth, async (req, res) => {
    try {
 ...
 */
app.patch("/api/v1/queues/:queueId", auth, async (req, res) => {
    try {
...
app.post("/api/v1/queues/:queueId/jobs/batch", auth, async (req, res) => {
    try {
        const body = z
            .object({
        ...
        const items = await prisma.$transaction(body.items.map((item) => {
            const schedule = computeJobSchedule({
...
app.get("/api/v1/jobs/:jobId/logs", auth, async (req, res) => {
    try {
        const { page, pageSize, skip, take } = parsePagination(req.query);
        const where = { jobId: req.params.jobId };
        const [total, items] = await Promise.all([
...
app.get("/api/v1/queues/:queueId/stats", auth, async (req, res) => {
    try {
        ...
        const [queued, scheduled, claimed, running, completed, deadLetter] = await Promise.all([
...
app.get("/api/v1/queues/:queueId/scheduled-jobs", auth, async (req, res) => {
    try {
        const { page, pageSize, skip, take } = parsePagination(req.query);
...
app.post("/api/v1/scheduled-jobs/:scheduledJobId/pause", auth, async (req, res) => {
    try {
...
app.post("/api/v1/scheduled-jobs/:scheduledJobId/resume", auth, async (req, res) => {
    try {
        const scheduledJob = await prisma.scheduledJob.update({
            where: { id: req.params.scheduledJobId },
            data: { isPaused: false }
 ...
 */
app.patch("/api/v1/scheduled-jobs/:scheduledJobId", auth, async (req, res) => {
    try {
 ...
 */
app.delete("/api/v1/scheduled-jobs/:scheduledJobId", auth, async (req, res) => {
    try {
        const existing = await prisma.scheduledJob.findUnique({
            where: { id: req.params.scheduledJobId }
        });
...
app.get("/api/v1/workers", auth, async (req, res) => {
    try {
        const { page, pageSize, skip, take } = parsePagination(req.query);
        const [total, items] = await Promise.all([
            prisma.worker.count(),
...
app.get("/api/v1/dead-letter", auth, async (req, res) => {
    try {
        const { page, pageSize, skip, take } = parsePagination(req.query);
        const [total, items] = await Promise.all([
